...
#
# @example
#   include moab::workloadmanager::server::config
class moab::workloadmanager::server::config () inherits ::moab::workloadmanager::server {

  $_config_dir = $::moab::workloadmanager::version ? {
    /^9[.]/ => '/opt/moab/etc',
    default => '/etc/moab'
  }
